...
def roomba_portrayal(agent):
    """Define visual appearance for each agent type."""
    if agent is None:
        return
...
model_params = {
    ...
    "num_obstacles": Slider("Number of Obstacles", 10, 0, 50),
    "num_dirty_tiles": Slider("Number of Dirty Tiles", 10, 10, 100),
    "max_steps": Slider("Maximum Steps", 1000, 0, 10000),
}
...
colors = ["tab:red", "tab:blue", "tab:purple", "tab:orange", "tab:cyan", 
          "tab:pink", "tab:olive", "tab:gray", "navy", "gold"]

roomba_movements_dict = {f"Roomba {i+1}": colors[i % len(colors)] for i in range(10)}
