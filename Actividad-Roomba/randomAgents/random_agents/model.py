...
class RandomModel(mesa.Model):
    """
    Roomba cleaning simulation with obstacles and charging stations.
    Agents discover environment boundaries through exploration.
    """
    def __init__(self, num_agents=1, num_obstacles=15, num_dirty_tiles=20, 
        ...
        border = [(x, y) for y in range(height) for x in range(width)
        ...
        if num_agents == 1:
            ...
            StationAgent(self, cell=charging_station_cell)
            RandomAgent(self, cell=charging_station_cell, home_station_pos=(1, 1))
        
        else:
            # Distribute charging stations across grid
            ...
            for i in range(num_agents):
                ...
                if available_zone_cells:
                    ...
                    StationAgent(self, cell=station_cell)
                    RandomAgent(self, cell=station_cell, home_station_pos=station_pos)

        # Place dirty floor tiles
        ...
        if len(available_cells) >= num_dirty_tiles:
            ...
            for cell in dirty_cells:
                FloorAgent(self, cell=cell, is_clean=False)
        ...
        # Setup data collection for up to 10 roombas
        roombas = [agent for agent in self.agents if isinstance(agent, RandomAgent)]
    ...
    def get_roomba_movements_by_index(self, roomba_index):
        if roomba_index in self.roomba_ids:
            agent_id = self.roomba_ids[roomba_index]
    ...
    def percentage_clean_tiles(self):
        return (self.count_clean_tiles() / (self.count_dirty_tiles() + self.count_clean_tiles())) * 100
